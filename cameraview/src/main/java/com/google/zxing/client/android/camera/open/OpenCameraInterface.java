        ...
        public static OpenCamera open(int cameraId)
            {
                ...
                if (index < numCameras)
                    {
                        Utils.showLog(TAG, "Opening camera #" + index);
                        camera = Camera.open(index);
                    } else
                    {
                        if (explicitRequest)
                            {
                                Utils.showLog(TAG, "Requested camera does not exist: " + cameraId);
                                camera = null;
                            } else
                            {
                                Utils.showLog(TAG, "No camera facing " + CameraFacing.BACK + "; returning camera #0");
