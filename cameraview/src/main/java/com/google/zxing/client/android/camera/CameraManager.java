...
public final class CameraManager
    {
        ...
        public synchronized void openDriver(SurfaceHolder holder, int width, int height) throws IOException
            {
                ...
                try
                    ...
                    {
                        // Driver failed
                        Utils.showLog(TAG, "Camera rejected parameters. Setting only minimal safe-mode parameters");
                        Utils.showLog(TAG, "Resetting to saved camera params: " + parametersFlattened);
                        // Reset:
                        if (parametersFlattened != null)
                            {
                                ...
                                try
                                    ...
                                    {
                                        // Well, darn. Give up
                                        Utils.showLog(TAG, "Camera rejected even safe-mode parameters! No configuration");
                                    }
                            }
                    }
        ...
        public Frame getFrame(byte[] data, int width, int height)
            {
                Frame toBeCapturedFrame = new Frame(width, height, Frame.DEPTH_UBYTE, 2);
                ((ByteBuffer) toBeCapturedFrame.image[0].position(0)).put(data);
                return toBeCapturedFrame;
            }
    }
